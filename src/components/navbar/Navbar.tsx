              ...
              {navLinks.map((link) => {
                ...
                if (link.hasDropdown) {
                  return (
                    ...
                    >
                      <button
                        className={`group relative flex items-center gap-1 px-3.5 xl:px-4 py-2 rounded-full font-bold text-xs xl:text-sm transition-all duration-300 cursor-pointer ${isActive || isServicesOpen
                              ...
                              {servicesList.map((item) => {
                                ...
                                return (
                                  <Link
                                    key={item.title}
                                    to={item.href}
                                    className={`flex items-start gap-3 p-2.5 rounded-xl transition-all duration-200 group/item border ${isSubActive
                                  ...
                                  >
                                    <div className={`p-2 rounded-lg transition-all duration-200 mt-0.5 ${isSubActive
                ...
                return (
                  <Link
                    key={link.label}
                    to={link.href}
                    className={`group relative font-bold text-xs xl:text-sm transition-all duration-300 px-3.5 xl:px-4 py-2 flex items-center gap-1.5 rounded-full ${isActive
                        ? 'text-white bg-gradient-to-r from-[#7A1F4D] via-[#8B1E5C] to-[#C2188B] shadow-md'
                        : 'text-[#222222] hover:text-[#8B1E5C] hover:bg-[#FAF8FB]'
                      }`}
                  >
                ...
                {navLinks.map((link) => {
                    ...
                    return (
                      <div key={link.label} className="rounded-2xl overflow-hidden border border-[#8B1E5C]/20 bg-[#FAF8FB]">
                        <button
                          ...
                          className={`w-full flex items-center justify-between p-3.5 text-left font-bold text-sm transition-colors ${isActive || isMobileServicesOpen
                            ...
                            {servicesList.map((item) => {
                              ...
                              return (
                                <Link
                                  key={item.title}
                                  to={item.href}
                                  className={`flex items-center gap-3 p-2.5 rounded-xl transition-colors group ${isSubActive
                  ...
                  return (
                    <Link
                      key={link.label}
                      to={link.href}
                      className={`flex items-center gap-2.5 p-3.5 rounded-2xl text-sm font-bold transition-all ${isActive
